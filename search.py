...
def depthFirstSearch(problem):
    ...
    while True:
        ...
        newState, movements = stack.pop()
        succList = problem.getSuccessors(newState)
        path.append(newState)
        for data in succList:
            coordinates = data[0]
            direction = data[1]
            if coordinates not in path:
                State = coordinates
                answer =  movements + [direction] 
                newTuple = (coordinates, answer)
                stack.push(newTuple)
...
class _RecursiveDepthFirstSearch(object):
    ...
    def RecursiveDepthFirstSearchHelper(self, node):
        ...
        succList = self.problem.getSuccessors(node)
        self.explored.add(node)
        succList = list(reversed(succList))
        for data in succList:
            coordinates = data[0]
            direction = data[1]
            if coordinates not in self.explored:
                answer = self.RecursiveDepthFirstSearchHelper(coordinates)
                if answer == True:
...
def depthLimitedSearch(problem, limit = 129):
    ...
    while True:
        ...
        newState, movements, current = stack.pop()
        print(current)
        if current > limit:
            break
        ...
        for data in succList:
            ...
            if coordinates not in path:
                State = coordinates
                answer =  movements + [direction] 
                newTuple = (coordinates, answer, current + 1)
                stack.push(newTuple)
    ...
    def RecursiveDepthLimitedSearchHelper(self, node):
        ...
        for data in succList:
            ...
            if coordinates not in self.explored:
                ...
                answer = self.RecursiveDepthLimitedSearchHelper(coordinates)
                if answer == True:
                    self.actions = self.actions + [direction]
                    return True
                else:
        ...
        if newState not in visited:
            ...
            for data in succList:
                ...
                if coordinates not in visited:
                    ...
                    answer =  movements + [direction] 
                    newTuple = (coordinates, answer)
                    priority = problem.getCostOfActions(answer)
                    priorityQueue.push(newTuple, priority)   
        ...
        if newState not in visited:
            ...
            for data in succList:
                ...
                if coordinates not in visited:
                    ...
                    answer =  movements + [direction] 
                    newTuple = (coordinates, answer)
                    Cost = problem.getCostOfActions(answer) + heuristic(coordinates, problem)
                    priorityQueue.push(newTuple, Cost)   
            visited.append(newState)   
